        ...
        <div class="card" style="width: 18rem;">
            ...
            <div class="card-body">
                ...
                @if(Auth::user())
                    ...
                    <p class="card-text">{{$car->price-($car->price*$user->discount/100)}} руб.</p>
                    @else
                        <p class="card-text">{{$car->price-($car->price*99/100)}} руб.</p>
                    @endif
                @elseif($car->discount==0)
                    <p class="card-text">{{$car->price}} руб.</p>
                @endif
                @if(Auth::user() && $car->discount!=0)
                    @if(($car->discount+$user->discount)<100)
                    <p class="card-text">Начальная цена {{$car->price}}</p>
                    <p class="card-text">Скидка {{$car->discount}}%</p>
                    <p class="card-text">Итого {{$car->price-($car->price*($car->discount+$user->discount)/100)}} руб.</p>
                    @else
                        <p class="card-text">Начальная цена {{$car->price}}</p>
                        <p class="card-text">Скидка {{$car->discount}}%</p>
                        <p class="card-text">Итого {{$car->price-($car->price*99/100)}} руб.</p>
                    @endif
                @elseif($car->discount!=0)
                    <p class="card-text">Начальная цена {{$car->price}}</p>
                    <p class="card-text">Скидка {{$car->discount}}%</p>
